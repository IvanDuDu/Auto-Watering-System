...
void addNewDevice(const char* mac,const char * token,const int humidity);
WiFiUDP udp;
...
void setup() {
    ...
    count_relayOn = storageManager.readMACsFromPROM(MACaddress,tokenAddress,Humidities);
...
void onMQTTMessage(char* topic, byte* payload, unsigned int length) {
    ...
    if (String(topic).startsWith("v1/devices/me/rpc/request/")) {
        ...
        const char* command = doc["params"]["command"];
        int cmdValue =(doc["params"]["cmdValue"]);

        

        Serial.printf("Received RPC:\n Method: %s\n BossID: %s\n DeviceToken: %s\n Command : %s\n cmdValue : %d\n ", 
                      method, bossID, deviceToken,command, cmdValue);
        ...
        if (strcmp(bossID,bossName)==0) {
            ...
            relayController.toggleWithDelay(relayIndex);
                ...
                Humidities[relayIndex] = cmdValue;
            }
...
void processUDPSensorData() {
    ...
    if (packetSize > 0) {
        ...
        // Parse packet: MAC//Token//VALUE/humidity
            ...
            if (strcmp(data, "Setting") == 0) {
                ...
                // Check if sensor value exceeds threshold
                    ...
                    if (macIndex >= 0) {
                        ...
                        if (sensorValue < Humidities[macIndex]) {
                    ...
                    // New device - add to memory and EEPROM
                        if (count_relayOn < MAX_PIN) {
                            addNewDevice(mac, token,atoi(humidity));
...
void addNewDevice(const char* mac, const char* token, const int humidity) {
    ...
    Humidities[count_relayOn]= humidity;

    count_relayOn++;
    
    // Save to EEPROM
    storageManager.writeMACsToPROM(MACaddress,tokenAddress,Humidities, count_relayOn);
